...
This component provides deep code analysis capabilities for research and quality checks.
It can be used as a CLI tool or integrated with other components.
"""
...
def analyze_code(codebase: Codebase, query: str) -> str:
    """Analyze code using the CodeAgent."""
    agent = CodeAgent(codebase)
    return agent.run(query)


def register_code_analyzer(app: CodegenApp) -> None:
    """Register code analyzer functions with the CodegenApp."""
    config = get_component_config("code_analyzer")
    
    @app.slack.event("message")
    async def handle_code_analysis_request(event: Dict) -> Dict:
        """Handle code analysis requests from Slack."""
        if "analyze" not in event.get("text", "").lower():
            return {"status": "ignored"}
        
        logger.info("[CODE_ANALYSIS] Received code analysis request")
...
@click.group()
def cli():
    """[bold blue]🔍 Code Analyzer CLI[/bold blue]
