...
# Usage: .\wsl-codegen-init.ps1 -WindowsPath "C:\Users\L\Desktop\WORKSPACE" -DistroName "Ubuntu" -WslPath "/home/l/Workspace"
...
wsl -d $DistroName -e mkdir -p $WslPath
if ($LASTEXITCODE -ne 0) {
...
if ($SetupSync) {
    # Check if windows-wsl2-sync.ps1 exists in the current directory
    $syncScriptPath = Join-Path -Path $PSScriptRoot -ChildPath "windows-wsl2-sync.ps1"
    if (-not (Test-Path -Path $syncScriptPath)) {
        Write-Log "Sync script not found at: $syncScriptPath" -Level "ERROR"
        Write-Log "Please download the windows-wsl2-sync.ps1 script first." -Level "ERROR"
        exit 1
    }
